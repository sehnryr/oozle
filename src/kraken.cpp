...
bool Leviathan_ReadLzTable(int32_t chunk_type,
                           const u_int8_t *src, const u_int8_t *src_end,
                           u_int8_t *dst, int32_t dst_size, int32_t offset,
                           u_int8_t *scratch, u_int8_t *scratch_end, LeviathanLzTable *lztable)
{
...
struct LeviathanModeRaw
{
    ...
    finline LeviathanModeRaw(LeviathanLzTable *lzt, u_int8_t *dst_start) : lit_stream(lzt->lit_stream[0])
    {
    }

    finline bool CopyLiterals(u_int32_t cmd, u_int8_t *&dst, const int32_t *&len_stream, u_int8_t *match_zone_end, size_t last_offset)
    {
    ...
    finline void CopyFinalLiterals(u_int32_t final_len, u_int8_t *&dst, size_t last_offset)
    {
        if (final_len >= 64)
        {
            do
...
struct LeviathanModeSub
{
    ...
    finline LeviathanModeSub(LeviathanLzTable *lzt, u_int8_t *dst_start) : lit_stream(lzt->lit_stream[0])
    {
    }
...
struct LeviathanModeLamSub
{
    ...
    finline LeviathanModeLamSub(LeviathanLzTable *lzt, u_int8_t *dst_start)
...
struct LeviathanModeSubAnd3
{
    ...
    finline LeviathanModeSubAnd3(LeviathanLzTable *lzt, u_int8_t *dst_start)
    {
...
struct LeviathanModeSubAndF
{
    ...
    finline LeviathanModeSubAndF(LeviathanLzTable *lzt, u_int8_t *dst_start)
    {
        for (size_t i = 0; i != NUM; i++)
            lit_stream[i] = lzt->lit_stream[(-(intptr_t)dst_start + i) & MASK];
    }
...
struct LeviathanModeO1
{
    ...
    finline LeviathanModeO1(LeviathanLzTable *lzt, u_int8_t *dst_start)
    {
...
bool Leviathan_ProcessLz(LeviathanLzTable *lzt, u_int8_t *dst,
                         u_int8_t *dst_start, u_int8_t *dst_end, u_int8_t *window_base)
{
    const u_int8_t *cmd_stream = lzt->cmd_stream,
                   *cmd_stream_end = cmd_stream + lzt->cmd_stream_size;
    const int32_t *len_stream = lzt->len_stream;
    ...
    const u_int8_t *copyfrom;
    u_int8_t *match_zone_end = (dst_end - dst_start >= 16) ? dst_end - 16 : dst_start;
...
bool Leviathan_ProcessLzRuns(int32_t chunk_type, u_int8_t *dst, int32_t dst_size, int32_t offset, LeviathanLzTable *lzt)
{
...
// Decode one 256kb big quantum block. It's divided into two 128k blocks
// internally that are compressed separately but with a shared history.
int32_t Leviathan_DecodeQuantum(u_int8_t *dst, u_int8_t *dst_end, u_int8_t *dst_start,
                                const u_int8_t *src, const u_int8_t *src_end,
                                u_int8_t *scratch, u_int8_t *scratch_end)
{
    const u_int8_t *src_in = src;
...
int32_t LZNA_DecodeQuantum(u_int8_t *dst, u_int8_t *dst_end, u_int8_t *dst_start,
                           const u_int8_t *src, const u_int8_t *src_end,
