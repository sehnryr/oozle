                     ...
                     MermaidLzTable *lz)
{
  ...
  n = Oozle_DecodeBytes (&out, src, src_end, &decode_count,
                         MIN (scratch_end - scratch, dst_size), false, scratch,
                         scratch_end);
  if (n < 0)
    return false;
  src += n;
  ...
  if (off16_count == 0xffff)
    {
      ...
      src += 2;
      off16_hi = scratch;
      n = Oozle_DecodeBytes (&off16_hi, src, src_end, &off16_hi_count,
                             MIN (scratch_end - scratch, dst_size >> 1), false,
                             scratch, scratch_end);
      if (n < 0)
        return false;
      src += n;
      ...
      off16_lo = scratch;
      n = Oozle_DecodeBytes (&off16_lo, src, src_end, &off16_lo_count,
                             MIN (scratch_end - scratch, dst_size >> 1), false,
                             scratch, scratch_end);
      if (n < 0)
        return false;
  ...
  if (tmp != 0)
    {
      ...
      n = Mermaid_DecodeFarOffsets (src, src_end, lz->off32_stream_1,
                                    lz->off32_size_1, offset);
      if (n < 0)
        return false;
      src += n;

      n = Mermaid_DecodeFarOffsets (src, src_end, lz->off32_stream_2,
                                    lz->off32_size_2, offset + 0x10000);
      if (n < 0)
        return false;
      src += n;
    }
  ...
  for (iteration = 0; iteration != 2; iteration++)
    {
      ...
      if (mode == 0)
        {
          src_cur = Mermaid_Mode0 (dst, dst_size_cur, dst_end, dst_start,
                                   src_end, lz, &saved_dist,
                                   (offset == 0) && (iteration == 0) ? 8 : 0);
        }
      else
        {
          src_cur = Mermaid_Mode1 (dst, dst_size_cur, dst_end, dst_start,
                                   src_end, lz, &saved_dist,
                                   (offset == 0) && (iteration == 0) ? 8 : 0);
        }
      if (src_cur == NULL)
          ...
          if (src_used < dst_count)
            {
              ...
              if (temp_usage > 0x40000)
                temp_usage = 0x40000;
              if (!Mermaid_ReadLzTable (
                      mode, src, src + src_used, dst, dst_count,
                      dst - dst_start, temp + sizeof (MermaidLzTable),
                      temp + temp_usage, (MermaidLzTable *)temp))
                return -1;
              if (!Mermaid_ProcessLzRuns (mode, src, src + src_used, dst,
                                          dst_count, dst - dst_start, dst_end,
                                          (MermaidLzTable *)temp))
                return -1;
            }
